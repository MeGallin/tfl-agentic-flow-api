...
class VictoriaAgent {
  constructor() {
    ...
    this.tools = new VictoriaLineTools();
    this.lineColor = '#0098D4'; // TFL Victoria line light blue
  }

  async processQuery(query, sharedLLM = null, context = {}) {
    console.log('[VictoriaAgent] Starting to process query...');
    ...
    try {
      ...
      console.log('[VictoriaAgent] LLM initialized, fetching TFL data...');
      ...
      if (arrivalQuery.isArrivalQuery && arrivalQuery.stationName) {
        console.log(
          `[VictoriaAgent] Detected arrival query for station: ${arrivalQuery.stationName}`,
        );
        ...
        console.log(
          `[VictoriaAgent] Station info result:`,
          JSON.stringify(stationInfo, null, 2),
        );

        if (stationInfo.station && stationInfo.arrivals) {
          console.log(
            `[VictoriaAgent] Found station: ${stationInfo.station.commonName} with ${stationInfo.arrivals.length} arrivals`,
          );
          ...
          console.log(
            `[VictoriaAgent] Formatted arrival data:`,
            JSON.stringify(arrivalData, null, 2),
          );
        } else {
          console.log(
            `[VictoriaAgent] No station or arrivals found in stationInfo`,
          );
        }
      }
      ...
      tflData = {
        line: {
          name: rawTflData.line?.name || 'Victoria Line',
          id: rawTflData.line?.id || 'victoria',
        },
        status: rawTflData.status?.slice(0, 3) || [
          { statusSeverityDescription: 'Good Service' },
        ],
        stationCount: rawTflData.stationCount || 16,
      ...
      console.log(
        '[VictoriaAgent] TFL data filtered and limited, preparing prompt...',
      );

      // Build system prompt with arrival data if available
      let arrivalInfo = '';
      console.log('[VictoriaAgent] Checking arrival data for prompt...');
      console.log(
        '[VictoriaAgent] tflData.arrivals:',
      ...
      if (
        ...
        console.log('[VictoriaAgent] Building arrival info for prompt...');
...
(Last updated: ${tflData.arrivals.lastUpdated})`;
        ...
        console.log('[VictoriaAgent] Arrival info constructed:', arrivalInfo);
      } else {
      ...
      console.log('='.repeat(80));
      console.log('[VictoriaAgent] Calling LLM...');
      ...
      console.log(
        '[VictoriaAgent] LLM response received, preparing final response...',
      );
      ...
      return {
        ...
        specialization: 'Victoria Line Underground Services',
      };
    } catch (error) {
      console.error('[VictoriaAgent] Victoria Agent error:', error);
      ...
      if (
        ...
        const fallbackResponse = `I'm experiencing some technical difficulties, but I can tell you that the next Victoria Line trains at ${stationName} are in ${nextArrivals.join(', ')} minutes.`;
      ...
      return {
        response:
          "I apologize, but I'm experiencing technical difficulties. As your Victoria Line specialist, I'm unable to provide information at the moment. Please try again shortly.",
        agent: 'VICTORIA',
