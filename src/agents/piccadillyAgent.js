...
const { PiccadillyLineTools } = require('../tools/piccadillyTools');
const { createPiccadillyPrompt } = require('../prompts/piccadillyPrompt');
const { todays_date_time } = require('../tools/dateTimeTools');

class PiccadillyAgent {
  constructor() {
    this.model = 'gpt-4o';
    this.temperature = 0.3;
    this.tools = new PiccadillyLineTools();
    this.lineColor = '#003688'; // TFL Piccadilly line dark blue
  }

  async processQuery(query, sharedLLM = null, context = {}) {
    console.log('[PiccadillyAgent] Starting to process query...');
    ...
    try {
      ...
      console.log('[PiccadillyAgent] LLM initialized, fetching TFL data...');
      ...
      if (arrivalQuery.isArrivalQuery && arrivalQuery.stationName) {
        console.log(
          `[PiccadillyAgent] Detected arrival query for station: ${arrivalQuery.stationName}`,
        );
        ...
        console.log(
          `[PiccadillyAgent] Station info result:`,
          JSON.stringify(stationInfo, null, 2),
        );

        if (stationInfo.station && stationInfo.arrivals) {
          console.log(
            `[PiccadillyAgent] Found station: ${stationInfo.station.commonName} with ${stationInfo.arrivals.length} arrivals`,
          );
          ...
          console.log(
            `[PiccadillyAgent] Formatted arrival data:`,
            JSON.stringify(arrivalData, null, 2),
          );
        } else {
          console.log(
            `[PiccadillyAgent] No station or arrivals found in stationInfo`,
          );
        }
      }
      ...
      tflData = {
        line: {
          name: rawTflData.line?.name || 'Piccadilly Line',
          id: rawTflData.line?.id || 'piccadilly',
        },
        status: rawTflData.status?.slice(0, 3) || [
          { statusSeverityDescription: 'Good Service' },
        ],
        stationCount: rawTflData.stationCount || 53,
      ...
      console.log(
        '[PiccadillyAgent] TFL data filtered and limited, preparing prompt...',
      );

      // Build system prompt with arrival data if available
      let arrivalInfo = '';
      console.log('[PiccadillyAgent] Checking arrival data for prompt...');
      console.log(
        '[PiccadillyAgent] tflData.arrivals:',
      ...
      if (
        ...
        console.log('[PiccadillyAgent] Building arrival info for prompt...');
...
(Last updated: ${tflData.arrivals.lastUpdated})`;
        ...
        console.log('[PiccadillyAgent] Arrival info constructed:', arrivalInfo);
      } else {
        console.log('[PiccadillyAgent] No arrival data available for prompt');
      }

      // Get current London time for the prompt
      const currentTime = todays_date_time();

      const systemPrompt = createPiccadillyPrompt(tflData, arrivalInfo, currentTime);

      console.log('[PiccadillyAgent] System prompt being sent to LLM:');
      console.log('='.repeat(80));
      console.log(systemPrompt);
      console.log('='.repeat(80));
      console.log('[PiccadillyAgent] Calling LLM...');
      ...
      console.log(
        '[PiccadillyAgent] LLM response received, preparing final response...',
      );
      ...
      return {
        ...
        specialization: 'Piccadilly Line Underground Services',
      };
    } catch (error) {
      console.error('[PiccadillyAgent] Piccadilly Agent error:', error);
      ...
      if (
        ...
        const fallbackResponse = `I'm experiencing some technical difficulties, but I can tell you that the next Piccadilly Line trains at ${stationName} are in ${nextArrivals.join(', ')} minutes.`;
      ...
      return {
        response:
          "I apologize, but I'm experiencing technical difficulties. As your Piccadilly Line specialist, I'm unable to provide information at the moment. Please try again shortly.",
        agent: 'PICCADILLY',
...
module.exports = { PiccadillyAgent };