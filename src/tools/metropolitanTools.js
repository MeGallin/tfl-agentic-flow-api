...
class MetropolitanLineTools {
  constructor() {
    this.tflApiUrl = process.env.TFL_API_BASE_URL || 'https://api.tfl.gov.uk';
    this.lineId = 'metropolitan';
    this.lineName = 'Metropolitan';
  }
  ...
  async getArrivals(stationId) {
    try {
      ...
      // Filter arrivals for Metropolitan Line only
      const metropolitanArrivals = data?.filter(arrival => arrival.lineId === this.lineId) || [];
      
      // Sort by expected arrival time
      metropolitanArrivals.sort((a, b) => a.timeToStation - b.timeToStation);

      return {
        stationId,
        line: this.lineName,
        arrivals: metropolitanArrivals,
        count: metropolitanArrivals.length,
        lastUpdated: DateTimeTools.getTFLTimestamp(),
      };
    } catch (error) {
...
module.exports = { MetropolitanLineTools };