...
class ElizabethLineTools {
  constructor() {
    this.tflApiUrl = process.env.TFL_API_BASE_URL || 'https://api.tfl.gov.uk';
    this.lineId = 'elizabeth';
    this.lineName = 'Elizabeth';
  }
  ...
  async getStationInfo(stationName) {
    try {
      ...
      const station = searchData.matches?.find(
        (match) => match.modes?.includes('elizabeth-line') || 
                   (match.modes?.includes('tube') && match.lines?.some(line => line.id === this.lineId))
  ...
  async getArrivals(stationId) {
    try {
      ...
      // Filter arrivals for Elizabeth Line only
      const elizabethArrivals = data?.filter(arrival => arrival.lineId === this.lineId) || [];
      
      // Sort by expected arrival time
      elizabethArrivals.sort((a, b) => a.timeToStation - b.timeToStation);

      return {
        stationId,
        line: this.lineName,
        arrivals: elizabethArrivals,
        count: elizabethArrivals.length,
        lastUpdated: DateTimeTools.getTFLTimestamp(),
      };
    } catch (error) {
...
module.exports = { ElizabethLineTools };